...
USERTYPE_USER           = 1
USERTYPE_SCRUMMASTER    = 2
USERTYPE_ADMINISTRATOR  = 3
...
class Person(db.Expando):
    ...
    usertype = db.IntegerProperty(default=USERTYPE_USER, required=True,
                                  choices=(USERTYPE_USER, USERTYPE_SCRUMMASTER, USERTYPE_ADMINISTRATOR))
    ...
    def put(self):
        for name in ('sreg', 'ax'):
            if hasattr(self, '_cache_%s' % name):
                ...
                import pickle
                setattr(self, name, pickle.dumps(value,
    ...
    def pretty_openid(self):
        return self.openid.replace('http://', '').replace('https://', ''
                ).rstrip('/').split('#')[0]

    def person_name(self):
        ax_dict = self.ax_dict()
        sreg_dict = self.sreg_dict()
        if ax_dict.get('firstname', False) and ax_dict.get('lastname',
                False):
    ...
    def type_of_user(self):
        if (self.usertype == USERTYPE_USER):
            return 'uporabnik'
        if (self.usertype == USERTYPE_SCRUMMASTER):
            return 'mojster skrama'
        if (self.usertype == USERTYPE_ADMINISTRATOR):
            return 'administrator'
        raise ValueError('Invalid value of usertype: ' + str(self.usertype))
    ...
    def __init__(
       ...
       session_id cookies are not guessable
        ...
        if key_name is None:
            ...
            import base64
            key_name = 'S'\
                 + base64.urlsafe_b64encode(key_name).rstrip('=')
        super(db.Expando, self).__init__(parent=parent,
                key_name=key_name, **kw)
