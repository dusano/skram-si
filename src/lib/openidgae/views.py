...
def render(template_name, request, response, extra_values={}):
    values = {'lip': openidgae.get_current_person(request, response)}

    values.update(extra_values)

    from django.template import loader, Context
    t = loader.get_template(template_name)
    return t.render(Context(values))
...
def args_to_dict(querydict):
    return dict([(arg, values[0]) for (arg, values) in
                querydict.lists()])
...
def MainPage(request, error_msg, template_name='openidgae-main.html'):
    response = HttpResponse()
    if request.method == 'GET':
        template_values = {'error_msg': error_msg}

        response['X-XRDS-Location'] = 'http://'\
             + request.META['HTTP_HOST'] + '/rpxrds/'
...
def OpenIDStartSubmit(request, default_success_url='/'):
    ...
    if request.method == 'POST' or request.method == 'GET':
        if request.method == 'POST':
            ...
            openid = 'https://www.myopenid.com/xrds'
            #openid = 'https://id.najdi.si/discovery'
        ...
        try:
            ...
            logging.error('OpenID discovery error with begin on %s: %s'
                           % (openid, str(e)))
            return show_main_page(request,
                                  'An error occured determining your server information.  Please try again.'
                                  )

        from openid.extensions import sreg
        sreg_request = sreg.SRegRequest(optional=['dob', 'gender',
                'postcode'], required=[
            'email',
            'nickname',
            'fullname',
    ...
    # finish URL with the leading "/" character removed
        ...
        parts[2] = \
            urlresolvers.reverse('openidgae.views.OpenIDFinish')[1:]
    ...
    # sometimes with long redirect urls
        ...
        response.write('<html><head><meta http-equiv="refresh" content="0;url=%s"></head><body></body></html>'
                        % (redirect_url, ))
        return response
    else:
...
def OpenIDFinish(request, default_success_url='/'):
    ...
    if request.method not in ('GET', 'POST'):
        ...
        url = 'http://' + request.META['HTTP_HOST']\
             + urlresolvers.reverse('openidgae.views.OpenIDFinish')
        session = openidgae.get_session(request, response)
        s = {}
        ...
        if auth_response.status == openid.consumer.consumer.SUCCESS:
            from openid.extensions import sreg
            sreg_response = \
                sreg.SRegResponse.fromSuccessResponse(auth_response)
            sreg_response = dict(sreg_response.iteritems())
            logging.debug('sreg_response: %r' % sreg_response)

            from openid.extensions import ax
            ax_response = \
                ax.FetchResponse.fromSuccessResponse(auth_response)
            ...
            if persons.count() == 0:
                ...
                for key in sreg_response:
                    if not p.sreg_dict().has_key(key)\
                         or p.sreg_dict()[key] != sreg_response[key]:
                        logging.debug('Setting sreg %s' % key)
                        p.sreg_dict()[key] = sreg_response[key]
                        changed = True
                for key in ax_items:
                    if not p.ax_dict().has_key(key) or p.ax_dict()[key]\
                         != ax_items[key]:
